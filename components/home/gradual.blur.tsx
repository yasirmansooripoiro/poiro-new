...
type ResponsiveVariantPrefix = "mobile" | "tablet" | "desktop";
type ResponsiveVariantKey =
  | "mobileHeight"
  | "tabletHeight"
  | "desktopHeight"
  | "mobileWidth"
  | "tabletWidth"
  | "desktopWidth";
type DimensionValue = GradualBlurProps[ResponsiveDimensionKey];

const debounce = <T extends DebouncedFn>(fn: T, wait: number) => {
...
const getResponsiveVariantKey = (
  prefix: ResponsiveVariantPrefix,
  key: ResponsiveDimensionKey,
): ResponsiveVariantKey =>
  `${prefix}${key.charAt(0).toUpperCase()}${key.slice(1)}` as ResponsiveVariantKey;
...
const useResponsiveDimension = (
  ...
  const [val, setVal] = useState<DimensionValue>(config[key]);
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v: DimensionValue = config[key];

      const mobileValue = config[getResponsiveVariantKey("mobile", key)];
      const tabletValue = config[getResponsiveVariantKey("tablet", key)];
      const desktopValue = config[getResponsiveVariantKey("desktop", key)];

      if (w <= 480 && mobileValue != null) v = mobileValue;
      else if (w <= 768 && tabletValue != null) v = tabletValue;
      else if (w <= 1024 && desktopValue != null) v = desktopValue;
